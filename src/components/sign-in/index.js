...
import {addNewToken} from "../../actions/token"
...
function HookSignIn() {
    ...
    useDispatch(addNewToken({token:token}));
    useDispatch(saveUser(info));
    ...
    const submit = async (e)=>{        
        e.preventDefault(); 
        if(email && password){
            const res = await axios.post("http://localhost:1234/api/auth/sign-in",{
                email:email,
                password:password,
            })
            const data = res.data;
            console.log(data)
            if(res.status == 200){
                // succees thì lưu token, user info, chuyển trang chủc
    ...
    const redirectOK = redirect?<Redirect to = "/" />:<></>
    return (
